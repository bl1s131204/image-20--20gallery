...
const STOPWORDS = new Set([
  ...
  'may', 'might', 'must', 'can', 'get', 'got', 'go', 'went', 'come', 'came'
]);

// Enhanced common tag patterns for better canonicalization
const COMMON_TAG_PATTERNS = {
  'forced feminization': ['forced femanizartion', 'forced femanization', 'forced feminzation', 'femanizartion', 'feminzation'],
  'crossdressing': ['cross dressing', 'cross-dressing', 'cross_dress', 'crossdresser', 'crossdress'],
...
function areRearrangements(tag1: string, tag2: string): boolean {
  ...
  return tokens1.every((token, index) => 
...
function tokenizeText(text: string): string[] {
  ...
  const cleanedTokens = expandedTokens
    ...
    .filter(token => {
      // Filter criteria
      return token.length >= FUZZY_CONFIG.minTokenLength && // Min length
             !STOPWORDS.has(token) && // Not a stopword
             !token.match(/^\d+$/) && // Not just numbers
             !['jpg', 'png', 'jpeg', 'webp', 'gif', 'bmp', 'tiff', 'svg'].includes(token); // Not file extensions
    });
...
export function extractTitleAndTags(filename: string): { title: string; tags: TagSource[] } {
  ...
  const titleWords = allTokens.slice(0, Math.min(3, Math.ceil(allTokens.length / 3)));
...
export function extractFolderTags(folderPath: string, userDefinedName?: string): TagSource[] {
  const extractedAt = new Date();
  const tagSources: TagSource[] = [];
...
export function extractMetadataTags(metadata?: { exif?: any; iptc?: any; xmp?: any }): TagSource[] {
  const extractedAt = new Date();
  const tagSources: TagSource[] = [];
  ...
  // Extract from EXIF
  if (metadata.exif) {
    const exifFields = ['ImageDescription', 'UserComment', 'Artist', 'Copyright'];
...
function selectCanonicalTag(
  ...
  members.forEach(member => {
    ...
    const hasUserSource = tagCounts.get(member)?.sources.some(s => 
      s.type === 'user_folder' || s.type === 'manual'
    );
    
    // Prefer user-defined tags
    if (hasUserSource && !tagCounts.get(bestCandidate)?.sources.some(s => 
      s.type === 'user_folder' || s.type === 'manual'
    )) {
...
export function normalizeAndGroupTags(allTagSources: TagSource[]): TagVariant[] {
  ...
  uniqueTags.forEach(tag => {
    ...
    uniqueTags.forEach(otherTag => {
      ...
      if (similarity >= FUZZY_CONFIG.minSimilarity && distance <= FUZZY_CONFIG.maxDistance) {
    ...
    Object.entries(COMMON_TAG_PATTERNS).forEach(([canonical, patterns]) => {
      patterns.forEach(pattern => {
        if (Array.from(group.members).some(member => 
          calculateSimilarity(member, pattern) >= FUZZY_CONFIG.minSimilarity
        )) {
  ...
  tagGroups.forEach((group, canonical) => {
    const aliases = Array.from(group.members).filter(tag => tag !== canonical);
    const totalCount = Array.from(group.members)
      .reduce((sum, tag) => sum + (tagCounts.get(tag)?.count || 0), 0);
...
export function processImageTags(
  ...
  const allTagSources: TagSource[] = [
    ...
    ...extractFolderTags(folderName || '', userFolderName).map(source => ({ ...source, imageId })),
  ...
  search(query: string): TagVariant[] {
    ...
    queryTokens.forEach(token => {
      ...
      for (const [indexToken, variants] of this.fuzzyIndex.entries()) {
        if (calculateSimilarity(token, indexToken) >= FUZZY_CONFIG.minSimilarity) {
    ...
    return Array.from(results).sort((a, b) => {
      ...
      const aExact = a.canonical === normalizedQuery || a.aliases.includes(normalizedQuery);
      const bExact = b.canonical === normalizedQuery || b.aliases.includes(normalizedQuery);
...
export type SortField = 'name' | 'title' | 'size' | 'date' | 'type' | 'tags' | 'relevance';
...
function calculateSearchRelevance(image: ImageData, query: string): number {
  ...
  const exactTagMatch = image.tags.some(tag => 
  ...
  const fuzzyTagMatch = image.tags.some(tag => 
    calculateSimilarity(tag.toLowerCase(), normalizedQuery) >= 0.7
  );
        ...
        if (source.type === 'filename') {
          ...
          const hasCommonToken = titleTokens.some(token => 
            sourceTokens.some(sourceToken => 
              calculateSimilarity(token, sourceToken) >= 0.8
            )
          );
